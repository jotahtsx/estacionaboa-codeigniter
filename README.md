...
* `www/`: Contém todos os arquivos da aplicação CodeIgniter.
* `docker-compose.yml`: Define os serviços Docker para a aplicação.
* `Dockerfile`: Configura o ambiente PHP e Apache para a aplicação.
* `Dockerfile.phpmyadmin`: Configura o ambiente phpMyAdmin para gerenciar o banco de dados.
...
️ Tabelas migradas
Você verá as seguintes tabelas criadas no banco de dados após as migrações:
...
Para confirmar, essas são as seguintes tabelas criadas no seu banco de dados:
